...
GITHUB_ID = "AuSoL1112"          
REPO_NAME = "escape_room_game"   
BRANCH_NAME = "main"            

BASE_URL = f"https://raw.githubusercontent.com/{GITHUB_ID}/{REPO_NAME}/{BRANCH_NAME}/"
...
class Game:
    ...
    def setup_room(self, room_number):
        ...
        self.state = 'explore'
        if room_number == 1:
            desk = InteractiveObject(self, 210, 472, 560, 192, "desk")
            painting = InteractiveObject(self, 400, 210, 350, 210, "painting")
            locked_door = InteractiveObject(self, 1050, 253, 250, 411, "room1_door")
            clock = InteractiveObject(self, 50, 210, 130, 472, "clock")
            ...
            chair = InteractiveObject(self, 550, 490, 250, 192, "chair")
            lock = InteractiveObject(self, 160, 367, 130, 113, "lock")
            large_painting = InteractiveObject(self, 370, 70, 600, 262, "large_painting")
            desk_pictures = InteractiveObject(self, 650, 411, 400, 175, "desk_pictures")
            self.all_sprites.add(chair, lock, large_painting, desk_pictures)
            self.objects.add(chair, lock, large_painting, desk_pictures)
            
        elif room_number == 3:
            grandfather_clock = InteractiveObject(self, 65, 122, 190, 595, "grandfather_clock")
            globe = InteractiveObject(self, 270, 420, 240, 262, "globe")
            desk_books = InteractiveObject(self, 570, 525, 400, 218, "desk_books")
            keypad = InteractiveObject(self, 1185, 310, 100, 87, "keypad") 

            self.all_sprites.add(grandfather_clock, globe, desk_books, keypad)
            self.objects.add(grandfather_clock, globe, desk_books, keypad)
            
        elif room_number == 4:
            lock = InteractiveObject(self,780, 437, 350, 306, "lock")
            desk = InteractiveObject(self, 280, 385, 880, 315, "desk")
            knight_armor_left = InteractiveObject(self, 170, 218, 240, 420, "knight_armor_left")
            knight_armor_right = InteractiveObject(self, 680, 218, 240, 420, "knight_armor_right")
        ...
        if self.current_room == 1:
            ...
            if object_name == "chair":
                if "의자 단서" not in self.room_inventories[self.current_room]:
                    self.show_dialogue(["낡은 의자다. 앉아볼까?", "앉으려고 보니 쪽지가 있다"," 기호 사용 예시: [1842] &@& [2225]= 1511"])
                    self.room_inventories[self.current_room].append("의자 단서")
                else:
                    self.show_dialogue("기호 사용 예시: [1842] &@& [2225]= 1511")
